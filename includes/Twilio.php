...
class Services_Twilio extends Services_Twilio_Resource
{
    ...
    $sid, $token, $version = '2010-04-01', Services_Twilio_TinyHttp $_http = null
    )
    {
        $this->version = $version;
        if (null === $_http) {
            $_http = new Services_Twilio_TinyHttp(
                "https://api.twilio.com", array("curlopts" => array(CURLOPT_USERAGENT => self::USER_AGENT))
            );
        }
    ...
    public function retrieveData($path, array $params = array())
    {
        ...
        return empty($params) ? $this->_processResponse($this->http->get($path)) : $this->_processResponse(
    ...
    {
        $path = "/$this->version/$path.json";
        return empty($params) ? $this->_processResponse($this->http->delete($path)) : $this->_processResponse(
    ...
    public function createData($path, array $params = array())
    {
        ...
        return empty($params) ? $this->_processResponse($this->http->post($path, $headers)) : $this->_processResponse(
                $this->http->post(
    ...
    private function _processJsonResponse($status, $headers, $body)
    {
        ...
        if ($decoded['code'] == 21212) {
            logActivity("SMS Manager - Twilio Error: Invalid From Number/Sender ID");
            return $decoded;
        }
        logActivity($body);
    }
    ...
    private function _processXmlResponse($status, $headers, $body)
    {
        ...
        (int) $decoded->Status, (string) $decoded->Message, (string) $decoded->Code, (string) $decoded->MoreInfo
        );
    }
}
